...
export default function Navbar() {
  ...
  // Obtenemos el usuario de Supabase
  const { user, signOut } = useAuth();
  
  // Obtenemos las funciones y variables del carrito
                  ...
                  <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-[#252525] rounded-xl shadow-2xl border border-gray-100 dark:border-gray-700 py-2 animate-fade-in-up origin-top-right z-50">
                    ...
                    <div className="py-1">
                      {user.user_metadata?.role === 'admin' ? (
                        <Link href="/dashboard" onClick={() => setIsProfileOpen(false)} className="flex items-center px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-white/5">
                          <LayoutDashboard size={16} className="mr-3 text-action-yellow" /> Panel Admin
                        </Link>
                      ) : (
                        <Link href="/perfil" onClick={() => setIsProfileOpen(false)} className="flex items-center px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-white/5">
                          <User size={16} className="mr-3 text-action-yellow" /> Mi Perfil
                        </Link>
                      )}
                      
                      <Link href="/configuracion" onClick={() => setIsProfileOpen(false)} className="flex items-center px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-white/5">
                        <Settings size={16} className="mr-3 text-gray-400" /> Configuración
                      </Link>
                    </div>

                    <div className="border-t border-gray-100 dark:border-gray-700 mt-1 pt-1">
                      <button onClick={handleSignOut} className="w-full text-left flex items-center px-4 py-2 text-sm text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors">
        ...
        {isMenuOpen && (
          <div className="md:hidden py-4 border-t border-gray-200 dark:border-gray-800 absolute left-0 right-0 bg-bone-white dark:bg-carbon-black shadow-xl z-40 px-4">
            ...
            <div className="flex flex-col space-y-2">
              ...
              {user ? (
                <>
                  <Link href={user.user_metadata?.role === 'admin' ? "/dashboard" : "/perfil"} onClick={() => setIsMenuOpen(false)} className="block px-4 py-3 rounded-lg font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800">
                    {user.user_metadata?.role === 'admin' ? "Panel Admin" : "Mi Perfil"}
                  </Link>
                  <button onClick={handleSignOut} className="w-full text-left block px-4 py-3 rounded-lg font-medium text-red-600 hover:bg-red-50 dark:hover:bg-red-900/10">
                    Cerrar Sesión
                  </button>
                </>
              ) : (
                <Link href="/login" onClick={() => setIsMenuOpen(false)} className="block px-4 py-3 rounded-lg font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800">
                  Iniciar Sesión
                </Link>
              )}
            </div>
